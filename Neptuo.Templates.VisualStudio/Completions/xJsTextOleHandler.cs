...
namespace Neptuo.Templates.VisualStudio.Completions
{
    ...
    internal sealed class JsTextViewCreationListener : IWpfTextViewConnectionListener
    {
        ...
        public async void SubjectBuffersConnected(IWpfTextView textView, ConnectionReason reason, Collection<ITextBuffer> subjectBuffers)
        {
            ...
            if (!subjectBuffers.Any(b => b.ContentType.IsOfType(TestContentTypeDefinition.ContentType)))
                return;

            var adapter = EditorAdaptersFactoryService.GetViewAdapter(textView);
            var filter = textView.Properties.GetOrCreateSingletonProperty<JsCommandFilter>("JsCommandFilter", () => new JsCommandFilter(textView, CompletionBroker));
    ...
    internal sealed class JsCommandFilter : IOleCommandTarget
    {
        ...
        public JsCommandFilter(IWpfTextView textView, ICompletionBroker CompletionBroker)
        {
            TextView = textView;
